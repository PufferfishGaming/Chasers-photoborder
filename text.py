...
def measure_text_width(text, font_size, fontpath, index) -> float:
    """Measure the pixel width of `text` at a given font size."""
    from PIL import Image as _Image, ImageDraw as _ImageDraw
    font = ImageFont.truetype(fontpath, font_size, index)
    draw = _ImageDraw.Draw(_Image.new("RGB", (1, 1)))
    return draw.textlength(text, font=font)


def get_optimal_font_size_constrained(texts, target_height, available_width,
                                      fontpath, index, max_font_size=100, min_font_size=1):
    ...
    while size > min_font_size:
        widest = max((measure_text_width(t, size, fontpath, index) for t in texts if t), default=0)
